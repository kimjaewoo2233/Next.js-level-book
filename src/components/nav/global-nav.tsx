...
export function GlobalNav() {
    ...
    return (
        <div className="fixed top-0 z-10 flex flex-col w-full bg-white border-b 
        border-gray-800 lg:bottom-0 lg:z-auto lg:w-72 lg:border-b-0 ogp'border-r lg:border-gray-800">
                            ...
                            <div key={section.name}>
                                ...
                                <div className="space-y-1">
                                    {section.items.map((item) => (
                                        <GlobalNabItem key={item.slug} item={item} close={close}/>
                                    ))}
                                </div>
                            </div>
                        )
...
function GlobalNabItem({ item, close }: { item: Item; close: () => false | void}) {
    return (
        <Link
            onClick={close}
            href={`/${item.slug}`}
            className="block px-3 py-2 text-sm font-medium text-gray-600 rounded-md hober:text-gray-800 hover:bg-gray-100"
        >
            {item.name}
        </Link>
    )
